...
if user_query: # If user input and press Enter, the following code will be triggered
    ...
    with st.spinner("🤖 Thinking... Generating response..."):
        ...
        if searchModeSelected == "Past Research":
            ai_response, think_text = ragMode.generate_ollama_response(user_query, isChroma=True, collection_name=collection_name)
        elif searchModeSelected == "PubMed":
            ai_response, think_text = ragMode.generate_ollama_response(user_query, isWebScrape=True)
        elif searchModeSelected == "Past Research + PubMed":
            ai_response, think_text = ragMode.generate_ollama_response(user_query, isWebScrape=True, isChroma=True, collection_name=collection_name)
